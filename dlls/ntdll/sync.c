...
{
    WARN("[LOL_DEBUG] FUNCTION RtlInitializeCriticalSection");
...
{
    WARN("[LOL_DEBUG] FUNCTION RtlInitializeCriticalSectionAndSpinCount");
...
{
    WARN("[LOL_DEBUG] FUNCTION RtlDeleteCriticalSection");
    crit->LockCount      = -1;
    crit->RecursionCount = 0;
    crit->OwningThread   = 0;
    if (crit_section_has_debuginfo( crit ))
...
{
    WARN("[LOL_DEBUG] FUNCTION RtlEnterCriticalSection");
    if (crit->SpinCount)
    {
        ULONG count;
...
{
    WARN("[LOL_DEBUG] FUNCTION RtlTryEnterCriticalSection");
    BOOL ret = FALSE;
...
{
    WARN("[LOL_DEBUG] FUNCTION RtlLeaveCriticalSection");
...
{
    WARN("[LOL_DEBUG] FUNCTION RtlAcquireSRWLockShared");
    union { RTL_SRWLOCK *rtl; struct srw_lock *s; LONG *l; } u = { lock };
...
{
    WARN("[LOL_DEBUG] FUNCTION RtlReleaseSRWLockExclusive");
    union { RTL_SRWLOCK *rtl; struct srw_lock *s; LONG *l; } u = { lock };
    union { struct srw_lock s; LONG l; } old, new;
...
{
    WARN("[LOL_DEBUG] FUNCTION RtlReleaseSRWLockShared");
    union { RTL_SRWLOCK *rtl; struct srw_lock *s; LONG *l; } u = { lock };
    union { struct srw_lock s; LONG l; } old, new;
